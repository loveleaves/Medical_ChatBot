...
def print_data_info(data_path, item_num=1):
    """
    function: 查看json数据格式
    """
    i = 0
...
class MedicalExtractor(object):
    ...
    def write_nodes(self, entitys: list, entity_type: str):
        """
        function: 写入实体到neo4j
        """
        ...
        for node in tqdm(set(entitys), ncols=80):
            ...
            try:
                ...
                print(e, "该条语句未成功执行: ", cql)

    def write_edges(self, triples: list, head_type: str, tail_type: str):
        """
        function: 写入实体之间的关系，前提是实体已经存在
        params:
            triples: 三元组列表，包含若干个三元组，每个三元组形式为[头实体,关系,尾实体]，
                如 triples=[['jack'，'has'，'adass'],['tom'，'has'，'sfewfs'],..]
            head_type: 头实体的类型
            tail_type: 尾实体的类型
        """
    ...
    def set_attributes(self, entity_infos: list, etype: str):
        """
        function: 写入实体的属性
        params:
            entity_infos: 实体属性信息
                如: entity_infos = [{'name':'tom','age':23,'sex':'male'},{'name':'jack','age':21,'sex':'male'},..]
            etype: 实体类型
        """
        print("写入 {0} 实体的属性".format(etype))
        for e_dict in tqdm(entity_infos[0:], ncols=80):
            name = e_dict['name']
            del e_dict['name']
            for k, v in e_dict.items():
                if k in ['cure_department', 'cure_way']:
                    ...
                    cql = """MATCH (n:{label})
                        WHERE n.name='{name}'
                        set n.{k}='{v}'""".format(label=etype, name=name.replace("'", ""),
                                                  k=k, v=v.replace("'", "").replace("\n", ""))
                try:
                    self.graph.run(cql)
                except Exception as e:
                    print(e, "该条语句未成功执行: ", cql)

    def merge_entity_and_attributes(self, entity_infos, etype):
        """
        function: 写入实体以及属性
        entity_infos:实体属性信息
            如: entity_infos = [{'name':'tom','age':23,'sex':'male'},{'name':'jack','age':21,'sex':'male'},..]
        etype:实体类型，如 etype='人物'
        """
        print("写入 {0} 实体".format(etype))
        for e_dict in tqdm(entity_infos[0:], ncols=80):
            cql = "merge(n:`{label}`{{{attributes_kv}}})"
            attributes_kv = ""
            for k, v in e_dict.items():
                kv = "{k}:'{v}'".format(k=k, v=v) + ","
                attributes_kv += kv
            cql = cql.format(label=etype, attributes_kv=attributes_kv.strip(','))
            try:
                self.graph.run(cql)
            except Exception as e:
                print(e, "该条语句未成功执行: ", cql)
    ...
    def export_data(self, data, path: str):
